...
export async function firstRunWizard() {
  ui.divider();
  ui.heading('🚀 Claude Anti-Slop Setup Wizard');
  ui.divider();
  console.log();
  ui.info('Welcome! Let\'s set up Claude Anti-Slop for your workflow.');
  console.log();
  ...
  if (stackIndex >= 0 && stackIndex < STACKS.length) {
    config.defaultStack = [STACKS[stackIndex].value];
    ui.success(`Selected: ${STACKS[stackIndex].label}`);
  } else {
    config.defaultStack = ['nextjs'];
    ui.warning('Invalid selection, using Next.js as default');
  }
  console.log();
  ...
  LOCALES.forEach((locale, i) => {
    console.log(`  ${i + 1}. ${locale.label}`);
  });
  const localeInput = await prompt('Enter locale number (1-7): ');
  const localeIndex = parseInt(localeInput) - 1;
  if (localeIndex >= 0 && localeIndex < LOCALES.length) {
    config.locales = [LOCALES[localeIndex].value];
    ui.success(`Selected: ${LOCALES[localeIndex].label}`);
  } else {
    config.locales = ['en'];
    ui.warning('Invalid selection, using English as default');
  }
  console.log();
  ...
  if (initMem.toLowerCase() !== 'n' && initMem.toLowerCase() !== 'no') {
    try {
      await initMemory({ force: false, verbose: false });
    ...
    ui.info('Memory initialization skipped');
  }
  console.log();
  ...
  if (installComp.toLowerCase() !== 'n' && installComp.toLowerCase() !== 'no') {
    try {
      ...
      console.log();
      ui.info(result.restartHint);
    } catch (error) {
      ui.error('Completion installation failed');
      ui.error(error.message);
    }
  } else {
    ui.info('Shell completion skipped');
  }
  console.log();
  ...
  console.log();
  ui.info('Documentation: https://github.com/mednabouli/claude-antislop');
  console.log();
