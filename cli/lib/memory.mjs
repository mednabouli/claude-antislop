...
export async function initMemory(options = {}) {
  ...
  if (!force && await fs.pathExists(MEMORY_DIR)) {
    if (verbose) console.log('Memory directory already exists');
    return { success: true, message: 'Memory already initialized' };
  }
  ...
  for (const category of VALID_CATEGORIES) {
    ...
    const readmeContent = `# ${category.charAt(0).toUpperCase() + category.slice(1)}\n\nStore your ${category} here.\n`;
...
export async function memorySearch(options = {}) {
  ...
  if (!await fs.pathExists(MEMORY_DIR)) {
    return { success: false, error: 'Memory directory not found. Run "claude-antislop install --init-memory" to initialize.', results: [] };
  }
  ...
  for (const cat of categories) {
    ...
    for (const file of files) {
      ...
      if (score > 0.3) {
        allResults.push({ file: `${cat}/${file}`, path: filePath, relevance: Math.round(score * 100), excerpt: content.substring(0, 500) + (content.length > 500 ? '...' : ''), category: cat, matchType: useFuzzy ? 'fuzzy' : 'exact' });
      }
    }
  }
...
export async function memoryWrite(options = {}) {
  ...
  const filePath = join(catDir, filename.endsWith('.md') ? filename : `${filename}.md`);
  ...
  return { success: true, file: `${category}/${filename}`, path: filePath, action: append ? 'appended' : 'created', size: content.length };
}
...
function calculateFuzzyScore(query, text) {
  ...
  for (const queryToken of queryTokens) {
    ...
    if (exactMatch) matchCount += 2;
    else if (partialMatch) partialMatchCount += 1;
  }
  const score = (matchCount * 2 + partialMatchCount) / (queryTokens.length * 2);
